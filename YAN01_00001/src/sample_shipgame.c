...
int processTarget(PlayerState* ps, Command* cmd)
{
  ...
  if ( ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col)] == SHIP2 )
  {
    ...
    ps->ship2Life--;
    __write(STDOUT, HIT_MSG, myStrLen(HIT_MSG));
    if (ps->ship2Life == 0)
    {
      __write(STDOUT, SHIP2_SUNK_MSG, myStrLen(SHIP2_SUNK_MSG));
    }
  }
  else if ( ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col)] == SHIP3 )
  {
    ...
    if (ps->ship3Life == 0)
    {
      __write(STDOUT, SHIP3_SUNK_MSG, myStrLen(SHIP3_SUNK_MSG));
    }
  }
  else if ( ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col)] == SHIP4 )
  {
    ...
    if (ps->ship4Life == 0)
    {
      __write(STDOUT, SHIP4_SUNK_MSG, myStrLen(SHIP4_SUNK_MSG));
    }
  }
  else if ( ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col)] == SHIP5 )
  {
    ...
    if (ps->ship5Life == 0)
    {
      __write(STDOUT, SHIP5_SUNK_MSG, myStrLen(SHIP5_SUNK_MSG));
    }
  }
  else
  {
    __write(STDOUT, MISSED_MSG, myStrLen(MISSED_MSG));
  }
...
int main(void)
{
  ...
  while(1)
  {
    if (curPlayer == 0)
    {
      __write(STDOUT, P0_PROMPT, myStrLen(P0_PROMPT)); 
    }
    else
    {
      __write(STDOUT, P1_PROMPT, myStrLen(P1_PROMPT)); 
    }
    ...
    if (ret == 0) //if its okay
    {
      ...
      {
        __write(STDOUT, EXIT_MSG, myStrLen(EXIT_MSG)); 
        //break;
      }
      ...
      {
        __write(STDOUT, HELP_MSG, myStrLen(HELP_MSG));
      }
      else if (bEnded)
      {
        __write(STDOUT, GAME_ALREADY_ENDED_MSG, myStrLen(GAME_ALREADY_ENDED_MSG));
      }
      ...
      else if (cmd.cmdNum == CMD_START)
      {
        if (bStarted)
        {
          __write(STDOUT, GAME_ALREADY_STARTED_MSG, myStrLen(GAME_ALREADY_STARTED_MSG));
        }
        else
        {
          bStarted = 1;
      ...
      else if (cmd.cmdNum == CMD_PLACE)
      {
        ...
        else
        {
          ...
          {
            __write(STDOUT, BAD_COMMAND_MSG, myStrLen(BAD_COMMAND_MSG));
          }
        }
      }
      else if (cmd.cmdNum == CMD_TARGET)
      {
        if (!bStarted)
        {
          __write(STDOUT, GAME_NOT_STARTED_MSG, myStrLen(GAME_NOT_STARTED_MSG));
        }
        else
        {
          if (curPlayer == 0)
          ...
          if (ret == 0)
          {
            ...
            if (curPlayer == 0)
            {
              ...
              {
                __write(STDOUT, YOU_WIN_MSG, myStrLen(YOU_WIN_MSG));
                bStarted = 0;
                bEnded = 1;
              }
            }
    ...
    else if (ret == -2) //__read error
    {
      __write(STDOUT, READ_ERROR_MSG, myStrLen(READ_ERROR_MSG));
      break;
    }
  }
