...
int send_string(int fd, char* s)
{
  ...
  return transmit_all(1, s, __libpov_strlen(s));
}
...
#define BASE 10
#define MAX_LEN 16
  char* s = __libpov_malloc(MAX_LEN + 1);
  __libpov_memset(s, '0', MAX_LEN);
  s[MAX_LEN] = '\0';
...
char* gen_string(long long n)
{
  ...
  unsigned alphabet_size = __libpov_strlen(alphabet);
  char* s = __libpov_malloc(n / alphabet_size + 1);
...
{
  send_string(1, "1\n");
  send_string(1, unsigned_to_base10_string(__libpov_strlen(sigs[i][0])));
  send_string(1, "\n");
