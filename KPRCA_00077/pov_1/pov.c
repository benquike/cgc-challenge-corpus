...
uint32_t ReadScreen(unsigned char** Lines)
{
  ...
  for (;;)
  {
    ...
    delimited_read(STDIN, &__read, &read_len, (unsigned char *)"\n", 1);
    Lines[LineCnt++] = __read;

    if (!strncmp("----", (const char *)__read, 4)  || !strncmp("Erro", (const char *)__read, 4))
      break;
    send_string(STDERR, (char *)__read);
  }
...
int main()
{
    ...
    int m = __strtol(s, NULL, 10);
    unsigned char secret[4];
    __memcpy(secret, (void *)&m, 4);
