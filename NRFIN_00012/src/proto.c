...
int recvdgram(int fd) {
    ...
    */
    ...
    __memset(curpkt,'\0', TFTTP_STRUCT_SIZE); 
    ...
    while(1) {
        ...
        __memcpy(&curfrag[f->seqid], f->data, MAX_FRAG_DATA_LEN);
        outstanding--;
        curfragid++;
        ...
        if (!outstanding) {
            ...
            __memcpy(curpkt->data, (char *)curfrag, MAX_PKT_SIZE);
...
void create_resp_pkt(tfttp_pkt* pkt, char *resp) {
    __memset(pkt,'\0', TFTTP_STRUCT_SIZE); 
    __strcpy((char *)pkt->data, resp);
    pkt->hdr.code = RESP;
    //for now, fixed length responses until pollers can support otherwise
    //pkt->size = sizeof(enum pkttype)+__strlen(resp)+1; 
    pkt->hdr.size = 256; //-_-
}
