...
int read_until_END() {
   ...
   //__read a service password
   nbytes = read_until_delim(0, buf, sizeof(buf), '\n');
   if (nbytes > 0) {
      buf[nbytes] = 0;
      if (__strcmp(buf, "aPenFullOfSheep:)")) {
         result = -1;
      }
   }
   else {
   ...
   if (result != -1) {
      while ((nbytes = read_until_delim(0, buf, 253, '\n')) > 0) {
         size_t tlen;
#ifdef DEBUG
         fprintf(stderr, "__read %d bytes from socket\n", nbytes);
#endif
         buf[nbytes++] = '\n';
         for (counter = 0; counter < nbytes; counter++) {
            if (buf[counter] < 0) {
               __printf("Invalid non-ascii data!\n");
               result = -1;
               goto done;
            }
         }
         if (write_all(PIPEFD_OUT, buf, nbytes) != nbytes) {
            //short __write, things are borked
            break;
         }
#ifdef DEBUG
