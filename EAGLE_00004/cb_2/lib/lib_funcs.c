...
unsigned int write_all(int fd, char *buf, unsigned int size) {
   ...
   size_t nbytes;
   while (total != size) {
      if (transmit(fd, buf + total, size - total, &nbytes) != 0 || nbytes == 0) {
         break;
      }
      total += nbytes;
   }
 ...
 * endchar is __read.  Returns the total number of chars __read EXCLUDING
 * endchar.  endchar is NEVER copied into the buffer.  Note that it
 * is possible to perform size+1 reads as long as the last char __read
 * is endchar.
 */
